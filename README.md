...
```mermaid
graph TB
    subgraph R1["リスク1: 信頼できないエージェントとのやり取り"]
        direction LR
        A1[ユーザー<br/>エージェント] -->|個人情報を送信| B1[航空会社<br/>エージェント❓]
        B1 -.->|実は偽物| E1[🔴 悪意の<br/>エージェント]
    end

    subgraph R2["リスク2: 間接的プロンプトインジェクションによる連鎖的な乗っ取り"]
        direction LR
...
AIエージェント間の対話をリアルタイムで仲介するエージェントを経由させ、計画外の行動を検知し改ざんされた命令の実行を防ぎます。各エージェントの通信はログで追跡可能とし、異常検知時には信頼スコアを自動減点してAIエージェント同士の対話を停止させます。
